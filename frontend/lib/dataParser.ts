...
// 风险等级归一化
// 业务语义：1=一级(最高), 2=二级(较高), 3=三级(中等), 4=四级(较低)
...
export function parseAndNormalizeData(
  ...
  // 新格式: raw_data 数组
  // 旧格式: data.list 数组
  const rawList = rawData.raw_data || rawData.list || [];
...
export async function loadDataFile(url: string): Promise<RawDataResponse> {
  ...
  // 新格式: raw_data 数组
  // 旧格式: list 数组
  const dataArray = data.raw_data || data.list;
  
  if (!dataArray || !Array.isArray(dataArray)) {
    throw new Error('Invalid data format: missing raw_data or list array');
  }

  // 新格式数据包含更多元信息，这里记录日志便于调试
  if (data.query?.date) {
    console.log(`[DataLoader] 加载数据日期: ${data.query.date}, 总记录数: ${data.summary?.total || dataArray.length}`);
  } else if (data.date) {
    console.log(`[DataLoader] 加载数据日期: ${data.date}, 总记录数: ${data.summary?.total || dataArray.length}`);
  }
  if (data.summary?.page_count) {
    console.log(`[DataLoader] 分页信息: ${data.summary.page_count} 页, 每页 ${data.page_size} 条`);
  }
