...
function serveDataFile(req: IncomingMessage, res: ServerResponse, next: () => void) {
  ...
  const filePath = candidates.find((candidate) => {
    ...
    return insideAllowedRoot && fs.existsSync(candidate) && fs.statSync(candidate).isFile()
  })
  ...
  res.statusCode = 200
  res.setHeader('Content-Type', DATA_MIME_TYPES[path.extname(filePath)] ?? 'application/octet-stream')
